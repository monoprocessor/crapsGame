...
class Craps(QMainWindow) :
    ...
    def restart(self):
        ...
        self.resultText = "The game has been reset."
        self.updateUI()
        self.rollingForLabel.setText("Restarted")
        if self.logging:
    ...
    def rollButtonClickedHandler ( self ):
        ...
        if self.currentBet > self.bank:
            self.resultText = "You don't have enough money to bet.."
            self.updateUI()
            return
        ...
        if self.currentBet < self.minimumBet:
            self.resultText = "This bet isn't enough."
            self.updateUI()
            return
